...
**`3.`** Download the provided file, **[`HW3_Template.Rmd`](https://blackboard.syracuse.edu/bbcswebdav/pid-8377480-dt-content-rid-95519473_1/xid-95519473_1)** 
...
**`0.`** Import `Box_Office_Mojo_Week3_HW3.csv` and save it as `mojo21` using the provided R code in Chunk 2.
...
After completing Part 1, the dataset, mojo_21_mod has
...
- **`mean_num1grossM`**, mean of the number 1 gross (num1grossM)
  - **`mean_num1grossM = mean(num1grossM, na.rm=T)`**
...
- to do this specify:
  - id_col=quartF, names_from=wkdayF, values_from=mean_num1grossM
...
- The zipped R Porject should be named **`HW 3 FirstName LastName`**.
