...
mojo_21_mod <- mojo_21_mod |>
  mutate(quartF = factor(quart, 
                         levels=c(1,2,3,4),
                         labels=c("1st Qtr", "2nd Qtr", 
                                  "3rd Qtr", "4th Qtr"))) |>
  glimpse()
...
# provided code has divided top10gross and num1 gross by 1 million
# values are rounded to 2 decimal place 

mojo_21_mod <- mojo_21_mod |>
  
  mutate(top10grossM = round(top10gross/1000000,2),
         num1grossM  = round(num1gross/1000000,2)) |>
...
mojo_qtr_wide <- mojo_qtr_smry |>
  pivot_wider(id_col=quartF, 
              names_from=wkdayF, 
              values_from=mean_num1grossM) |>
  rename(Qtr = quartF) 
...
(mojo_qtr_barplot <- mojo_qtr_long |>
   ggplot() +
   geom_bar(aes(x=Qtr, y=mean_num1grossM, fill=Day), 
            stat="identity", position="dodge") + 
   theme_classic())
...
(mojo_qtr_barplot +
  ...
  guides(fill = guide_legend(nrow = 1)) +             # puts legend in 1 row
  ...
  scale_fill_brewer(palette = "Spectral"))            # changes color palette
